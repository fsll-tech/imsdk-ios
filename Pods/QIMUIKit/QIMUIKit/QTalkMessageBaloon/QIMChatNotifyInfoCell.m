...
+ (CGFloat)getCellHeightWithMessage:(QIMMessageModel *)message chatType:(ChatType)chatType {
    ...
    MDHTMLLabel * label = [[MDHTMLLabel alloc] initWithFrame:CGRectMake(0, 0, kCellWidth, 20)];
    label.text = message.message;
    label.font = [UIFont systemFontOfSize:12];
//    [label sizeToFit];
    return [label size].height + 20;
    //去特么的第三方，算的高度是错的
}
...
- (void)refreshUI {
    ...
    self.htmlLabel = [[MDHTMLLabel alloc] initWithFrame:CGRectMake(0, 0, kCellWidth, 20)];
...
//    self.htmlLabel.numberOfLines = 0;
    ...
    self.htmlLabel.textAlignment = MDHTMLLabelVerticalAlignmentCenter;
...
//
    self.htmlLabel.activeLinkAttributes = @{@"color":@"#0000FF"};
...
//    CGFloat height = [MDHTMLLabel sizeThatFitsHTMLString:self.message.message withFont:[UIFont systemFontOfSize:12] constraints:CGSizeZero limitedToNumberOfLines:0];
    CGSize titleSize = [self.htmlLabel size];
