...
- (void)qim_uploadMyPhotoData:(NSData *)headerData withCallBack:(QIMKitUploadMyPhotoCallBack)callback {
    ...
    NSURLSessionDataTask *task = [manager POST:destUrl parameters:dict constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
- (void)privateUpLoadImage:(NSData *)fileData WithMsgId:(NSString *)key WithMsgType:(int)type WithPathExtension:(NSString *)extension withCallBack:(QIMKitUploadImageCallBack)callback {
    ...
    NSURLSessionDataTask *task = [manager POST:destUrl parameters:dict constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
            ...
            NSLog(@"%@",uploadProgress);
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:kQIMUploadImageProgress object:@{@"ImageUploadKey":fileName, @"ImageUploadProgress":@(uploadProgress.completedUnitCount/uploadProgress.totalUnitCount)}];
            });
        } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            //上传成功
            NSLog(@"上传成功");
